...
namespace SandloDb.Core
{
    public class SandloDbContext
    {
        ...
        public int Remove<T>(T entity) where T : class, IEntity
        {
            ...
            if (_collections[type] is not ConcurrentBag<T> collection)
            {
                throw new InvalidOperationException(nameof(collection));
            }
            ...
            var updateCollection = new ConcurrentBag<T>(collection.Where(e => e.Id != entity.Id));

            _collections.TryUpdate(type, updateCollection as ConcurrentBag<object> ?? throw new InvalidOperationException(nameof(updateCollection)), collection as ConcurrentBag<object> ?? throw new InvalidOperationException(nameof(collection)));
        ...
        public IList<T> GetAll<T>() where T : class, IEntity
        {
            ...
            return _collections[type] is not ConcurrentBag<T> collection ? new List<T>() : collection.ToList();
        }
        ...
        public IList<T> GetBy<T>(Func<T, bool> predicate) where T : class, IEntity
        {
            ...
            return _collections[type] is not ConcurrentBag<T> collection ? new List<T>() : collection.Where(predicate).ToList();
        }
        ...
        public T? Get<T>(Func<T, bool> predicate) where T : class, IEntity
        {
            ...
            return _collections[type] is not ConcurrentBag<T> collection ? null : collection.FirstOrDefault(predicate);
        }
        ...
        public T? GetById<T>(Guid id) where T : class, IEntity
        {
            ...
            return _collections[type] is not ConcurrentBag<T> collection ? null : collection.FirstOrDefault(e => e.Id == id);
        }
